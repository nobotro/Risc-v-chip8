...
uint8_t x = 0; //cursor x
uint8_t y = 0;  //cursor y
...
void to_string(uint16_t number,char s[])
{
...
void draw_pixel(uint8_t x,uint8_t y,uint8_t pixel){

		*(SCREEN_ADDRESS+(y*64) +x) = pixel;
}
void printr(char str[],uint32_t delay)
{
	    ...
	    for(uint8_t i =0;i<8;i++) // line number
		{
			for(uint8_t j =0;j<8;j++)  //pixel per line
			{
					 
					draw_pixel(x+j,y+i,(font8x8_basic[*c-32][i]>>j)&1);
